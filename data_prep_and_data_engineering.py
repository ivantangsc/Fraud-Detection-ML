...
logging.info("Data loaded  ---%.2f seconds ---" % (starttime - timecheck0))


#                                               DATA ENGINEERING
df_bets = datacleaning(df_bets)
timecheck1 = time.time()
logging.info("Datacleaning  ---%.2f seconds ---" % (timecheck1 - timecheck0))
df_bets = side_related_price_and_profit(df_bets)
timecheck2 = time.time()
logging.info("Side related Features  ---%.2f seconds ---" % (timecheck2 - timecheck1))
df_bets = client_roi(df_bets)
timecheck3 = time.time()
logging.info("Client ROI ---%.2f seconds ---" % (timecheck3 - timecheck2))
df_bets = match_and_bet_time_difference(df_bets)
timecheck4 = time.time()
logging.info("Match time bet time differences ---%.2f seconds ---" % (timecheck4 - timecheck3))
df_bets = category_encoding(df_bets)
timecheck5 = time.time()
logging.info("Category names encoded  ---%.2f seconds ---" % (timecheck5 - timecheck4))
df_bets = bet_selection_features(df_bets)
timecheck6 = time.time()
logging.info("Bet Selection Features  ---%.2f seconds ---" % (timecheck6 - timecheck5))
df_bets = onehour_avg_price(df_bets)
timecheck7 = time.time()
logging.info("1 hour avg price  ---%.2f seconds ---" % (timecheck7 - timecheck6))
...
timecheck9 = time.time()
logging.info("TOTAL TIME  ---%.2f seconds ---" % (timecheck9 - starttime))
...
def data_sampling (df_bets, fraud_betids, remove_periods):
